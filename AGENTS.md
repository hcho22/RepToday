...
- **Second, in-progress PRD - anonymous product telemetry:** `.claude/agent/tasks/prd-funnel-instrumentation_260803.md` (`US-T##`, tracked the same way); US-T01 (transport spike), US-T02 (the analytics seam), US-T03 (the Convex sink), US-T05 (the anonymous per-install identity on `AppState`), and US-T04 (the live fire-and-forget `URLSession` transport that joins them) have landed; the emission call sites that would trigger any of it are still ahead.
...
Services/Analytics/         Telemetry sinks: the live Convex POST plus the inert no-op fallback
