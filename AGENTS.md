...
- **Second, in-progress PRD - anonymous product telemetry:** `.claude/agent/tasks/prd-funnel-instrumentation_260803.md` (`US-T##`, tracked the same way); US-T01 (transport spike), US-T02 (the analytics seam), US-T03 (the Convex sink), and US-T05 (the anonymous per-install identity on `AppState`) have landed, emission call sites and the client transport between the seam and the sink are still ahead.
...
**Monorepo:** `ios/` is the whole MVP; `convex/` is the telemetry sink only - one append-only `events` table, one internal `logEvent` mutation, and one `POST /logEvent` HTTP action (US-T03) that is the sink's single entry point, deliberately dumb with no analysis in it, and reached by nothing yet since the client transport is US-T04; there is still **no backend behind the core loop**, which stays fully on-device and offline. Standard Convex layout, so the npm project (`package.json`) sits at the repo root and the functions in `convex/` - see `convex/README.md`. `proxy/` is a deploy-ready but unwired Cloudflare Worker for the deferred Variety Language LLM slice (holds the Anthropic key, stores nothing, receives only two pillar values).
