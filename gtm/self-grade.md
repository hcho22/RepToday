...
3. **The waitlist is live on the site, but the page is unpublished and per-source tagging is not yet guaranteed.** Both landing variants now embed the founder's Kit email-capture form (a hero card plus a closing CTA), so K0's waitlist-dependent signals have a real capture destination rather than a fabricated one; what stays open is publishing the page and configuring per-source/UTM tagging in the Kit dashboard, which the static HTML cannot itself guarantee.
