...
| L-3 | MUST-FIX | "There is nothing to sell" is falsified by the measurement plan | FIXED for the objection as raised - both variants' data FAQ now: never sold, on-device / private iCloud, planned anonymous per-install usage events, waitlist-email disclosure, aligned with `06-channels/event-metric-schema.md` so the FAQ, the schema, and the future nutrition label tell one story. **The one-story part re-opened after this run:** US-T06 shipped an opt-out the schema now documents and the FAQ still omits, so the three no longer match. The FAQ was left unedited on purpose (live A/B test), and reconciliation is tracked as a Legal item on `pre-publication-checklist.md`, blocking submission of any build that actually collects |
...
- The site's data FAQ dropped the false absolute "there is nothing to sell" for a disclosure that matched the event schema at the time, on both variants. (It no longer matches: US-T06 later made collection opt-out, and the FAQ does not mention the off switch. Deliberately not edited mid-A/B-test - see the Legal item on `pre-publication-checklist.md`.)
