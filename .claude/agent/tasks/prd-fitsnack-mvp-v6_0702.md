...
- [x] Onboarding's basics step collects height as **feet + inches** and weight in **lbs** (replacing the cm/kg sliders), with sensible US defaults, >= 44pt touch targets, and VoiceOver values - *defaults are `5 ft 8 in` / `175 lb`, near the US adult median and deliberately **not** the values the validation test types, so a conversion that ignored its input could not pass by coincidence. Both controls the step has carried miss the app's own 44pt floor - a `Slider` lays out at 20pt and the platform `Stepper`'s halves at 46.5 x 32pt, under the floor in the dimension a thumb is least accurate in - so the row draws its own 44pt-square step buttons that step on release and repeat while held (the repeat being the one thing a hand-drawn stepper otherwise gives up against the platform one), and Age moved onto the same row because the miss was never specific to the units. The repeat is driven by `Utilities/PressRepeater.swift`, which is the only thing that turns a press into steps: the button's two gestures (a press *tracker* and a drag) each report one half of a press and neither commits on its own, so a press has exactly one source of increments. The step lands on **release**, as it does everywhere else in the app - a finger that touches `+` on its way to scrolling the step commits nothing, and neither does a press the scroll view steals - while a press held past the hold delay repeats instead and its release adds nothing on top of what the read-out showed. Both gestures measure "still on the button" with the one `PressRepeater.pressTolerance` (half the touch target, roughly the button's own bounds), so a thumb's ordinary roll still steps on release while a finger lifted away from the control commits nothing; the release is read off a drag rather than a `TapGesture` precisely because a tap's ~10pt is not configurable, and the two radii disagreeing dropped the step silently. The repeat stops the moment the clamped value refuses to move rather than spinning at the end of the range, and because nothing commits until the finger lifts, the glyph tints to the accent while pressed - cleared on release, on cancel and on disappear, unanimated under Reduce Motion - so the press is acknowledged at all. `PressRepeaterTests` gates all of it, which is the touch path the accessibility assertions never reach. The row is a single **adjustable** element to VoiceOver rather than two buttons: it reads "Height, 5 feet 8 inches" and a swipe moves it by that row's own step (1 in, 5 lb). The spoken value is a sentence whose nouns agree with their own counts ("1 foot 1 inch") - the screen's terse `5 ft 8 in` read aloud is letters - which is the agreement rule the active-session prescription copy already follows. At an accessibility type size the row stacks rather than squeezing, and the +/- glyph's growth is capped so it cannot draw through its own button*
