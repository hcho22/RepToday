...
- [x] The seeded per-set target is reflected in the planned wall-clock: `SessionAssembly.workSecondsPerSet` prices a set as a fixed per-set setup cost plus the per-unit work of the target actually prescribed (a hold's per-unit cost is one second per prescribed second, so its setup is the authored `estimate - defaultDurationSeconds`; a rep's is `secondsPerRep`, capped at the movement's own authored `estimate / defaultReps`), so a x1.30 first session is *sized* as the longer session it is - by the work the extra reps really cost rather than by scaling the setup along with them - and the ±1 minute timing-fit promise stays honest. The seeded **set count** is the pre-fit starting volume only - the fit may still trade sets against the duration the user asked for, and the requested duration always wins
