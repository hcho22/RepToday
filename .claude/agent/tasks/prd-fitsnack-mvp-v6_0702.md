...
**Status:** Epics A-C built (`[x]`); Epics D-N complete (US-D01-N05 done); Epic O (post-v6 refinements, US-O01-O04) added and complete (US-O01-O04 done, `[x]`).
...
- [x] Elapsed time is always visible; touch targets are >= 60pt on this screen - *elapsed time is derived from an injected clock and re-read once a second through a `TimelineView` (accurate and backgrounding-resilient, frozen at completion); every interactive control uses `Theme.Spacing.workoutTouchTarget` (60pt).* **The visibility half is deliberately reversed by US-O03:** *the always-on clock is gone from the player and the rest overlay, and the total is revealed once, on the completion summary. The measurement is untouched - `startedAt` / `elapsed(asOf:)` still feed the log, Default Duration learning, and the Consistency Score - so only the display changed. The 60pt touch-target half still holds*
...
- [x] Accessibility: VoiceOver labels on controls, Dynamic Type respected - *every control carries a VoiceOver label, the demo and target/set trackers are combined accessibility elements, and all type comes from Dynamic-Type `Theme.Typography` tokens. The spoken target agrees with every count it names (a single-set warm-up or cooldown reads "1 set of a 45 second hold", not "1 sets of 45 second holds", and the agreement composes with the US-O02 per-side suffix), and the Ready Screen's lineup rows call the player's `targetText`/`targetAccessibilityText` rather than carrying a second copy - so the preview speaks the same sentence as the player instead of reading the "×" glyph aloud*
...
- **Expected Result:** Each exercise shows its demo and target, and completing sets advances to the next exercise. (Since US-O03 there is deliberately no running clock on this screen; the total appears on the completion summary.)
- **Failure Indicator:** The player loses position, the demo does not play (with no static fallback), or a running session clock is back on the screen.
...
- [x] The swapped exercise carries a fresh capacity-relative target and keeps the slot inside its own time budget - *the substitute is materialized by `ExerciseSwap` with a fresh `AdaptiveOverload` target and the original slot's rest and set count. The set count is the one thing a swap may move, and only when it has to: because Step 6 sizes a target from demonstrated capacity, a peer the user has never logged opens at its own default however grown the slot it replaces is, so the substitute's set count may be re-picked inside the assembler's own `minTrainingSets...maxTrainingSets` rails - and only on the blocks the assembler itself adjusts, never the warm-up or the cooldown - to bring the slot back within the slot's tolerance of the planned seconds it replaces (`slotToleranceSeconds` flat on a slot the assembler would re-fit; widened by a share of the estimated work on a non-adjustable bookend, which has neither rest nor lever). Set-preservation is the first selection criterion, so drift is compared only among candidates that fit the slot as built: the original count wins whenever *any* in-band peer is already in budget at it, and a re-picked candidate is reachable only when none is. The view model resets the set counter to 1 and discards any sets already recorded for the replaced movement (it is being abandoned, mirroring a skip), so a swapped exercise never carries stale sets into the eventual log and a user part-way through a longer slot is never stranded past the end of a shorter substitute* **[Superseded by US-CC03/US-CC04, 2026-08-14: under the even-round circuit model a swap now *keeps* the block's uniform round count and no longer re-picks a slot's set count (a re-pick would make the circuit uneven); a substitute joins at that round count or declines, and the widened `slotTolerance` applies to every slot since none has a set lever. See the Continuous-Circuit PRD and ADR-0003.]**
...
- **Expected Result:** The session resumes at exercise 3, set 2, with elapsed time preserved. (Since US-O03 elapsed time is no longer displayed mid-session, so it is read off the completion summary's total rather than the top bar.)
...
- [x] The seeded per-set target is reflected in the planned wall-clock: `SessionAssembly.workSecondsPerSet` prices a set as a fixed per-set setup cost plus the per-unit work of the target actually prescribed. A hold's per-unit cost is known a priori - one second per prescribed second, *per side* for an `isPerSide` movement - so its setup is the authored remainder `estimate - sides × defaultDurationSeconds`; a rep has no such anchor, so the setup is the assumed half (`setupSecondsPerSet`, capped by `maxSetupShareOfEstimate`) and the cadence is read off the movement's own `(estimate - setup) / defaultReps`. Assuming the setup rather than the cadence is what keeps per-side and slow-tempo movements honest in both directions, and calibrating that constant from the holds' *observed* remainders rather than from rep entries it would then assign zero setup to is what keeps it non-circular. So a x1.30 first session is *sized* as the longer session it is - by the work the extra reps really cost rather than by scaling the setup along with them - and the ±1 minute timing-fit promise stays honest. The seeded **set count** is the pre-fit starting volume only - the fit may still trade sets against the duration the user asked for, and the requested duration always wins
...
- [x] An in-session swap keeps the session's seed: `WorkoutEngineProtocol.swapExercise` carries the `sessionPolicy`, so `ExerciseSwap` sizes a substitute with the same Step 6 levers (`progressionRate`, Start Seed, `varietyWindow`, and the Return / Re-entry Ramp ease read off the session's own `wasReturn` stamp) the lineup was built on instead of reverting to the neutral defaults, and checks the substitute's time budget through the session's own `SessionAssembly.workSecondsPerSet` model at the target each side actually carries
...
- [x] The always-on elapsed clock is removed from the player top bar and the rest-overlay top bar; no running session clock is visible during the session - *both top bars now share one `SessionTopBar` carrying the close control and nothing else, so there is no second place for a clock to reappear. Asserted on the surface rather than in the source: `PerSideSwapEvidenceTests.testHoldTimerReplacesTheAlwaysOnSessionClock` and `testRestOverlayCarriesItsCountdownButNoSessionClock` read the live accessibility tree of the hosted production player and rest overlay and fail on any `"Elapsed time …"` label*
- [x] The completion summary still shows the total session duration (the existing `completedDurationMinutes()` -> `SessionSummary` path is unchanged), so the total is revealed only at the end - *the clock is hidden, not removed: `startedAt` / `elapsed(asOf:)` are untouched, so the log, Default Duration learning (US-F04) and the Consistency Score read exactly what they did before, gated by `testTotalDurationStillReachesTheCompletionSummary`. The two halves are also gated together on one hosted surface by `PerSideSwapEvidenceTests.testTheSessionTotalIsHiddenDuringPlayAndRevealedOnTheCompletionSummary`, which reads the live player for an elapsed clock (and for the minutes worked so far in any form), activates the real "Finish session" control, and reads the summary that replaces it for the same session's total - so a player with no clock and a summary with no total, which is a session whose minutes simply vanished, fails rather than passing as two half-truths*
- [x] Timed (`isHold`) exercises get a per-exercise Hold Timer: the user taps "Start hold", a countdown runs, and at zero it fires the `RestTimerFeedback` cue **exactly once** and auto-records the set, advancing to rest. Rep-based exercises are unchanged (set tracker + "Complete set", no timer). The hold is counted **one side at a time**, because the engine charges a per-side movement for both sides (US-O02): a set of `core_side_plank` is two 20-second legs, and a single countdown that recorded the set at the end of one would quietly halve the work the session was planned around - exactly the failure the per-side target copy exists to prevent. So a per-side hold cues the *switch* at the end of the first leg and parks on side 2, where the tap to start it is the user's own time to change position; only the last leg records the set. Four of the library's seventeen holds are per-side (`core_side_plank`, `mobility_9090_hip`, `mobility_pigeon`, `mobility_kneeling_hip_flexor`), so this is the ordinary case, not an edge one. The button (`Start hold (side 2 of 2)`) and the set tracker (`Side 2 of 2`) both name it. The timer is the offer rather than the only way through: the secondary row keeps a quiet manual completion beside Swap and Skip, titled by the same `completeButtonTitle` logic as every other slot, so a user interrupted part-way through a multi-set plank - or one who held it off-timer - can bank the work instead of losing it to a Skip, which discards the exercise's recorded sets. It stays offered **while a leg runs** too, not only between legs. Hiding it there left a user 25 seconds into set 3 of a plank with exactly two visible ways out - "Stop hold", which records nothing, and "Skip", which silently discards the two sets already banked - so a non-destructive one has to be on screen in every state. Tapping it takes the running countdown down without firing the cue (coming out early is the user's choice, not the timer's verdict) and banks exactly one set through the same `completeSet` path the countdown takes at zero, resetting the side so a per-side set is never left parked on side 2 of a set already logged. That gives up the tidiness of "while the leg runs, the countdown is the only thing that records a set"; removing a silent data-loss path is worth more than the rule
- [x] The Hold Timer uses an absolute deadline with pause/resume wired to `scenePhase` (backgrounding-resilient), persists what a resume must not lose, and its cue never fires early, per-tick, or while paused - *the countdown itself is a shared `Countdown` value type (absolute deadline, `pause`/`resume`, `hasElapsed`) that **both** timers run on. Writing the Hold Timer as a copy of the rest timer meant every fix landed on one and survived in the other, so the duplication was the defect, not a tidiness complaint. What each timer does at zero stays distinct in the view model: a hold has sides and banks a set, a rest has extend/skip and only cues.*

  *Where the two genuinely differ is what survives the player being torn down, and getting that wrong is the mistake this story kept paying for. A **rest** legitimately continues while the app is away - the user is resting - so it restores and keeps counting; one that already ran out is simply over, not something to fire a cue about. A **hold does not**: the user stopped holding when they left the screen. So a leg is **never persisted and never restored** - only the side they still owe is carried, and a resumed session always comes back idle offering "Start hold". Modelling the hold on the rest timer instead produced the same phantom set through three separate doors (a deadline already in the past, a frozen remainder of zero, and a frozen remainder that `onAppear` obligingly un-froze); each was patched and the next appeared. Carrying only the side is both the honest model and the one with no door left to guard. In-session backgrounding still freezes and resumes the leg through `scenePhase`, which is the interruption the user is actually present for.*

  *`ActiveSessionState.Hold` is optional and now carries a single field, so a snapshot written before this story **and** one written by the earlier US-O03 shape both decode to the side they recorded - an in-progress session is never lost to an app update, and the running leg such a snapshot carries is exactly the thing that must not come back. The player also holds `isIdleTimerDisabled` while a session is being played: without it the screen locks under a planking user, the hold pauses, and the cue that ends it never lands - the one failure mode this story's own promise could not survive. It is released on **completion as well as dismissal**, since the player is not dismissed when the session ends. A hold started in the window between requesting a swap and the engine answering is refused (`canStartHold` considers `isSwapping`) and cleared outright by the substitution that lands, so a countdown can never run out against a substitute and record a set the user never performed*
- [x] Uses `Theme` tokens (60pt active-screen targets) with VoiceOver labels; no XP/levels/badges; Verify in iOS Simulator - *the countdown ring is now one `CountdownRing` shared by the rest overlay and the Hold Timer, so the two timers a user meets in a session read as the same object, and it is drawn inside the demo's own card (one shared `exerciseSlotCard()`, the demo's exact `ExerciseDemoView.height`) so starting a hold changes what is in the slot and nothing else - not the card under it, not the exercise name and target below it. The secondary row's slots are decided by the exercise rather than by whether a hold is running, so a running leg dims controls in place instead of removing them and Skip never slides across the row under the user's thumb. Those slots are narrow - three columns on a timed movement - and the manual completion carries the row's longest title ("Finish exercise"), so it wraps and the row grows rather than clipping the words that say what the tap does: 60pt is the touch-target floor, never a fixed height. Verified in the Simulator three ways: rendered PNGs of the production player at a hold (idle, running, idle at accessibility Dynamic Type, per-side side 1 and side 2), the rest overlay, and the completion summary that finally reports the total, **written by `PerSideSwapEvidenceTests` itself** into `artifacts/reports/us-o03/` - the renders and their assertions run on every test run, but where the bytes land is opt-in: a plain test run writes to a per-run temporary directory so the worktree stays clean, and `xcodebuild -project ios/RepToday/RepToday.xcodeproj -scheme RepToday -destination 'platform=iOS Simulator,name=iPhone 16' test -only-testing:RepTodayTests/PerSideSwapEvidenceTests REPTODAY_WRITE_EVIDENCE=1` regenerates the committed images in place (a build setting rather than a shell variable, because a Simulator-hosted test bundle inherits no shell environment - the scheme forwards it, empty by default; the repo path is resolved from the test's own `#filePath` rather than a machine-specific temporary path, and `REPTODAY_EVIDENCE_DIR` overrides it outright - superseded: `REPTODAY_EVIDENCE_DIR` now names a *root* with the story folder appended, not the final directory, and every evidence suite resolves through the shared `EvidenceOutput` helper, see `EvidenceOutput` and CLAUDE.md for the current rule), so a layout or copy change shows up as a diff instead of leaving a stale render cited as evidence, and the running-hold capture pins the countdown to a specific remaining second first so that diff is the screen changing rather than where the runloop pump landed; the live accessibility tree read off those same hosted surfaces; and two live wall-clock tests that drive the real controls the way VoiceOver's double-tap does - `testHoldTimerRunsDownAndHandsOffToRestInTheLivePlayer` lets a short hold actually run out and asserts the player recorded the set and handed off to the rest overlay, and `testBankingASetByHandMidLegEndsTheLegAndOpensTheRestInTheLivePlayer` activates "Start hold" and then the manual completion mid-countdown, asserting the leg comes down, the set is banked, and the rest opens with the counter advanced one set*
...
- [x] A pure `UnitConversion` utility converts lbs<->kg and feet-inches<->cm, unit-tested for known values (5 ft 7 in ~ 170 cm; 165 lb ~ 74.8 kg) and round-trips - *`Utilities/UnitConversion.swift`, on the exact international definitions (1 in == 2.54 cm, 1 lb == 0.45359237 kg), so the conversions round-trip to the nearest display unit across the whole range the UI offers - every inch of 47...87 and every pound of 70...445, not just the two values the criteria name. Those bounds are the imperial cover of the 120...220 cm / 35...200 kg sliders they replace, deliberately rounded outward at every end (47 in == 119.4 cm, 87 in == 221.0 cm, 70 lb == 31.8 kg, 445 lb == 201.8 kg): with no profile-edit surface yet, a range narrower than the one it replaced would silently clamp somebody's weight on the way in and under-report every HealthKit energy estimate for the life of the account. `feetAndInches(fromCentimeters:)` rounds **total** inches and re-splits, so a height that rounds up to a whole foot reads `6 ft 0 in` rather than the `5 ft 12 in` that splitting first and rounding second produces; that is the one arithmetic bug this conversion has, and it is gated*
- [x] Onboarding's basics step collects height as **feet + inches** and weight in **lbs** (replacing the cm/kg sliders), with sensible US defaults, >= 44pt touch targets, and VoiceOver values - *defaults are `5 ft 8 in` / `175 lb`, near the US adult median and deliberately **not** the values the validation test types, so a conversion that ignored its input could not pass by coincidence. Both controls the step has carried miss the app's own 44pt floor - a `Slider` lays out at 20pt and the platform `Stepper`'s halves at 46.5 x 32pt, under the floor in the dimension a thumb is least accurate in - so the row draws its own 44pt-square step buttons that step on release and repeat while held (the repeat being the one thing a hand-drawn stepper otherwise gives up against the platform one), and Age moved onto the same row because the miss was never specific to the units. Only the *drawing* is hand-rolled: the halves are real `Button`s in a custom `ButtonStyle`, so the single step is the platform's own touch-up **inside the button's own bounds**, decided by the same hit testing every other control in the app gets, touch slop and all. A finger that touches `+` on its way to scrolling the step commits nothing, neither does a press the scroll view steals, and neither does one carried clear of the button before it lifts; a press stays with the button it *landed* on, so a small drift over the adjacent `-` still steps `+` and never `-`, which is how two adjacent buttons behave anywhere else in iOS. Deciding that in the view is what this control got wrong twice - two gestures enforcing different radii dropped the step silently, and the drift radius that replaced them was drawn around where the finger *landed*, which is not a button's bounds - so the geometry left the view entirely. That leaves `Utilities/PressRepeater.swift` the one job the platform has no notion of: a press held past the hold delay repeats instead of stepping once, and the release ending it adds nothing on top of what the read-out showed. It runs off the style's own press state - the same signal that tints the glyph - so the repeat cannot outlive a press the view never heard end. The repeat stops the moment the clamped value refuses to move rather than spinning at the end of the range, and because nothing commits until the finger lifts, the glyph tints to the accent while pressed - in over 100ms and out over 200ms, since a tap is usually shorter than SwiftUI's 0.35s default; cleared on release, on cancel and on disappear, unanimated under Reduce Motion - so the press is acknowledged at all. `PressRepeaterTests` gates all of it, which is the touch path the accessibility assertions never reach. The row is a single **adjustable** element to VoiceOver rather than two buttons: it reads "Height, 5 feet 8 inches" and a swipe moves it by that row's own step (1 in, 5 lb). The spoken value is a sentence whose nouns agree with their own counts ("1 foot 1 inch") - the screen's terse `5 ft 8 in` read aloud is letters - which is the agreement rule the active-session prescription copy already follows. At an accessibility type size the row stacks rather than squeezing, and the +/- glyph's growth is capped so it cannot draw through its own button. Because a hosted window can read the screen but not deliver a touch, the presses themselves are asserted in the shipping app by `OnboardingImperialUITests` in a `RepTodayUITests` bundle - launched, walked to the step, and pressed by coordinate - run under its own `RepTodayUITests` scheme so the documented `-scheme RepToday test` stays the unit run it has always been. That bundle also walks the Validation Test below as an automated run: the measurements it names are dialled in with the step buttons, the rest of the flow is answered on its defaults - including the Health share prompt the first launch ends on - and onboarding has to land on a generated session*
- [x] `OnboardingViewModel` converts the imperial inputs to `heightCm`/`weightKg` in `buildUser`; the persisted `UserProfile` stays metric and the HealthKit energy path still receives kilograms - *the imperial answers are the only stored state and `heightCm`/`weightKg` are **derived**, so there is no second representation to drift out of sync. The HealthKit half is asserted end-to-end rather than by inspection: a 60-minute session at 5 MET for a 165lb user must estimate ~374 kcal, with a second assertion pinning it below `5 x 165` outright, because reading the pound value as kilograms inflates every calorie written to Health by 2.2x and nothing downstream would flag it*
- [x] Uses `Theme` tokens; Verify in iOS Simulator - *every color, font, dimension and the 44pt target come from `Theme`; verified by hosting the production `OnboardingView` in a real Simulator key window and reading its live accessibility tree (`OnboardingBasicsEvidenceTests`), which is what caught both the under-size touch target and a label VoiceOver was speaking twice. Renders at the default and largest accessibility type sizes are committed under `artifacts/reports/us-o04/`*
...
- **Build:** `xcodegen generate` then `xcodebuild ... -scheme FitSnack ... test`; single `FitSnack` scheme builds the app and runs `FitSnackTests`. *(As built, post-rebrand: the `RepToday` scheme builds the app and runs `RepTodayTests`, and since US-O04 a second `RepTodayUITests` scheme runs the XCUITest bundle out of process so the default run never waits on an app launch - see `CLAUDE.md`.)*
