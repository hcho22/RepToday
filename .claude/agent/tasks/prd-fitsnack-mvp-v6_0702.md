...
- [x] The Hold Timer uses an absolute deadline with pause/resume wired to `scenePhase` (backgrounding-resilient), persists in `ActiveSessionState`, and restores on resume; the cue never fires early, per-tick, or while paused - *it mirrors the rest timer's contract exactly (`ActiveSessionState.Hold` alongside `.Rest`, optional so a snapshot written before this story still decodes and an in-progress session is never lost to an app update). The side is persisted **independently of whether a leg is running**, so a user who finished side 1 and backgrounded the app comes back owing side 2 rather than silently repeating the first and doing three legs of a two-leg set. **Leaving the screen at all freezes the leg**: closing the player pauses the hold exactly as backgrounding does, so what is written to disk is a frozen remainder rather than a deadline that will be in the past by the time the user taps Resume. Restore is defensive about that regardless of how the snapshot was written - a leg still marked running whose deadline has already gone by (an OS kill with no scene-phase transition, a snapshot from before this, a resume days later) comes back **idle on the side the user still owes**, offering "Start hold" again, rather than as a live countdown whose first tick would fire the cue and bank a set nobody performed. Since a paused countdown can never fire its cue, the player now also holds `isIdleTimerDisabled` while a session is being played: without it the screen locks under a planking user, the hold pauses, and the cue that ends it never lands - the one failure mode this story's own promise could not survive. It is released on **completion as well as dismissal**, since the player is not dismissed when the session ends and the celebration screen has no countdown to protect. A hold started in the window between requesting a swap and the engine answering is refused (`canStartHold` considers `isSwapping`) and cleared outright by the substitution that lands, so a countdown can never run out against a substitute and record a set the user never performed*
