...
- [x] The completion summary still shows the total session duration (the existing `completedDurationMinutes()` -> `SessionSummary` path is unchanged), so the total is revealed only at the end - *the clock is hidden, not removed: `startedAt` / `elapsed(asOf:)` are untouched, so the log, Default Duration learning (US-F04) and the Consistency Score read exactly what they did before, gated by `testTotalDurationStillReachesTheCompletionSummary`*
