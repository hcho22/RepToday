...
- [x] The `POST /logEvent` HTTP action bounds `installId`'s length. US-T03 shipped it as an unbounded `v.string()` - reviewed and consciously accepted there, because nothing was sending it yet - which leaves it the one client-controlled field with no ceiling while `props` is capped at 32 keys / 4096 UTF-8 bytes, so a hostile client could write rows up to Convex's ~1MB document limit through it. The bound belongs in the story that first puts a real identifier on the wire: a short cap sized for US-T05's UUIDv4, rejected with the same `400`-and-no-insert as the sink's other rejections, and no format or UUID-shape check beyond it.
  ...
  *Both closed, in one treatment, as the criterion asks. `installId` is bounded at **64 UTF-8 bytes** (`MAX_INSTALL_ID_BYTES`), sized for US-T05's 36-character UUIDv4 with room for a differently-shaped anonymous id, and it is a **length** bound only - `"not-a-uuid"` still lands, because policing the identifier's shape would forbid a later story from changing it, and the bound counts UTF-8 rather than UTF-16 for the same reason the `props` cap does. The second gap is closed by a **request-size check ahead of everything**, on the raw bytes before the body is even parsed (`MAX_REQUEST_BODY_BYTES`, 64 KiB): an order of magnitude above the 4096-byte `props` cap, so it never pre-empts the mutation's more specific rejection - a 5 KiB bag still comes back naming the byte limit it actually broke - and far below any argument bound, so serialization can no longer be reached by size at all. Both answer the same `400`-and-no-insert as every other caller fault.*
