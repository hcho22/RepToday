...
- [x] **No new Swift Package dependency is added.** `ios/RepToday/project.yml` still lists Lottie as the only third-party package; `convex-swift` is **not** added. Only the deployment `.site` URL is new config (a build setting or plist value), alongside the install id from US-T05 and the opt-out flag from US-T06. - *`project.yml` is untouched by this story. The URL is an `Info.plist` value, `RepTodayAnalyticsEndpoint`, holding the deployment `.site` **origin** with the `/logEvent` route appended in code, so the route's name stays owned by the code that knows the contract. The shipped value points at the **dev** deployment `courteous-dogfish-560`; **no production deployment has been chosen** - which account and plan host it is still an Open Question below - and moving it is a one-line configuration change. The `convex/` side did gain devDependencies (`convex-test`, `vitest`, `@edge-runtime/vm`) for the boundary suite under criterion 7; that is the root npm project, which reaches no iOS target, and this criterion is about the iOS one.*
