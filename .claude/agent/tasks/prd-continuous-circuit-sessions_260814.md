...
- **Skip inside a circuit** removes the exercise from every remaining round (the rotation passes over a skipped station), which is the only aggregate-logging-correct reading of the existing "skip this exercise" contract once the walk rotates - so it lands here rather than waiting for US-CC07. **Swap** still keeps the slot's set count (unchanged US-K03) and works mid-circuit without breaking the rotation or the label; reconciling a swap *across all remaining rounds* (and reshaping the block's round-rest to re-absorb a swap's drift) remains US-CC07.
