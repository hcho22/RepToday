...
- Status: Complete (all US-CC stories landed). US-CC01 (auto-advancing work window for strength sets), US-CC03/US-CC04 (the even-round engine timing model: uniform set count per training block plus the two-gap transition/round-rest fit, engine-only), US-CC02 (circuit rotation of the training block, "Round N of M", player-only), US-CC05 (hands-free warm-up/cooldown bookend holds - auto-start plus the per-side "Switch sides" beat, player-only), US-CC06 (quiet in-flow escape hatches with an explicit user Pause, player-only), US-CC07 (skip/swap apply to all remaining rounds - Option A honest late-entrant, player-only; skip was already aggregate-correct from US-CC02 and swap needed no engine change because the substitute keeps the block's uniform round count), US-CC08 (generous runtime pace - `workSecondsPerSet` becomes the runtime window as well as the planning number via `workPaceGenerosityFactor = 1.25`, engine-only; the player needed no new value because the coupling was already structural from US-CC01), US-CC09 (hands-free completion-logging invariant - auto-advanced and Done-advanced sets log completed, only an explicit Skip logs not-done, and `CompletedSet` stays pinned to `reps`/`durationSeconds`; hardening/lock-in with no production change because the invariant was already structural from US-CC01/US-CC02/US-CC07), US-CC10 (non-verbal per-state audio cues - a `SessionCue` vocabulary via `SessionCuePlayer` marking go/halfway/transition/round-rest/done by distinct tones, ducking rather than stopping the user's own audio, VoiceOver-coordinated to withhold the tone while still firing the haptic; player-only), US-CC11 (visual-primary work window - a static movement illustration plus name, target, and compact countdown ring together in the work window with a prominent "Next: <exercise>" transition beat, the US-O01 Lottie fast-follow seam preserved and Reduce Motion honored; player-only), US-CC12 (migration - an old paused session resumes in the new player from its saved position, uneven pre-CC03 blocks mapped deterministically into round/station terms with no work lost or double-counted and the hold restored idle; hardening/lock-in with no production change because the migration was already structural once US-CC02 kept `currentStepIndex`/`currentSet` unchanged), US-CC13 (one-time dismissible first-run explainer for the self-driving session, held on a US-CC06 user pause while shown, player-only), and **US-CC14** (accessibility acceptance for the auto-advancing flow / **+ More time** first-class - `extendActiveCountdown` adds the shared +15s to a live work window or hold and is a labeled 60pt VoiceOver control on both, the countdown ring carries `.updatesFrequently` so VoiceOver polls the remaining time on demand rather than announcing every tick, Dynamic-Type wrapping and a Reduce-Motion-stilled ring; player/presentation-only, no engine/timing/persistence/advance/completion-logging/cue change) have landed - **the continuous-circuit PRD is complete**. Decisions locked with the captain 2026-08-14; US-CC07's completed-round-count semantics settled as Option A (honest late-entrant) 2026-08-15; US-CC08's generosity factor and its two accepted consequences (fewer rounds, and the US-M03 desk-worker bias now binding) confirmed 2026-08-15.
- Story prefix: `US-CC##`.
...
- [x] A persisted `ActiveSessionState` written by the old manual player (US-K04) loads and resumes in the new continuous-circuit player from its saved position (current step index, current set, completed sets, skips, session-clock origin). - *structural: `init(state:)` copies `completedSets`/`skippedStepIDs` verbatim, restores `startedAt`, and clamps `currentStepIndex`/`currentSet` into range; proved by `testResumesAnOldUnevenPausedSnapshotPreservingWorkSkipsAndHoldSide`*
- [x] Because the old snapshot may carry uneven per-exercise set counts (pre-US-CC03), the resumed player maps the saved position into round/station terms deterministically without losing completed work or double-counting a set; any per-side `hold.side` restores idle per the US-O03 rule. - *`blockSpan` reads the block's round count "M" as the **max** station set count and `hasSet(_:inRound:)` gates each station by its own count, so a shorter station drops out of the later rounds; `hold.side` restores `holdSide` idle (never a running leg). Same test asserts `currentRound == 2`/`circuitRoundCount == 3`, `completedSetCount == 3` (none lost/double-counted), the skip intact, and `holdSide == 2` with `isHolding == false`; `testResumedUnevenPreCircuitPlaysToCompletionWithoutLosingWork` drives a resumed uneven circuit to completion without trapping or dropping banked work*
- [x] No migration writes a new snapshot schema that the prior build could not read back in the same session (forward safety); the `exercisedMinutes`/`hold` optional-and-defaulted fields decode unchanged. - *no schema change made; `PersistenceTests.testActiveSessionStateDecodesWithoutTheHoldOrExercisedMinutesFields` decodes the oldest resumable shape (neither key present) unchanged*
- [x] Typecheck, lint, and the `RepToday` unit suite pass (test: an old-shape `ActiveSessionState` resumes with completed work preserved).
...
**Implementation note (2026-08-17, hardening/lock-in - no production change):**
Tracing the resume path confirmed the migration was **already structural** once US-CC02 landed, so this story shipped as a lock-in story with no engine, player, or snapshot-schema change.
US-CC02 deliberately kept `ActiveSessionState`'s `currentStepIndex`/`currentSet` unchanged - a station's r-th set *is* round r - so an old paused snapshot's saved position maps straight into round/station terms with nothing to convert.
`init(state:)` copies `completedSets` and `skippedStepIDs` verbatim, restores the `startedAt` clock origin, and clamps the position into range, so no completed work is lost or double-counted at construction.
The uneven-set-count case (pre-US-CC03) is handled by `blockSpan`, which reads the block's round count "M" as the **max** station set count, and `hasSet(_:inRound:)`, which gates each station by its own set count - so a shorter station simply drops out of the rounds beyond it, and the position-driven rotation stays total-preserving from the resumed spot rather than trapping or replaying lost work.
The one piece of per-side state that survives is the *side*: the leg itself is never persisted (US-O03), so a resumed training hold comes back **idle** on the owed side (`holdSide`, `isHolding == false`, `canStartHold`) awaiting a deliberate Start-hold tap rather than a phantom running countdown (a resumed *bookend* hold auto-starts the owed side fresh, US-CC05).
Forward safety is inherent: no schema field was added, and the `hold`/`exercisedMinutes` fields stay optional-and-defaulted, so the oldest resumable shape decodes unchanged.
Deliverables: the PRD validation test `ActiveSessionViewModelTests.testResumesAnOldUnevenPausedSnapshotPreservingWorkSkipsAndHoldSide` (uneven pre-CC block, mid-block position, some completed sets, one skipped exercise, `hold.side == 2`), the walk-to-completion guard `testResumedUnevenPreCircuitPlaysToCompletionWithoutLosingWork`, the forward-safety decode `PersistenceTests.testActiveSessionStateDecodesWithoutTheHoldOrExercisedMinutesFields`, and the `docs/test-coverage.md` row.
...
- [x] **+ More time** is present on every auto-advancing work window and hold, always extends the current window by a meaningful increment, and is reachable by VoiceOver (a labeled, hittable control at the project's touch-target size for active screens, 60pt).
- [x] The auto-advancing countdown does not steal VoiceOver focus or fire rapid per-second VoiceOver announcements; remaining time is available on demand (e.g. an accessibility value the user can query) rather than continuously spoken.
- [x] Dynamic Type: the movement name, rep target, round label, and control labels scale with Dynamic Type without truncation or overlap at the largest accessibility sizes.
- [x] Reduce Motion: the countdown ring and any transition animation degrade to a non-animated indicator when Reduce Motion is on (the ring may show remaining time as a static value/step rather than a sweeping animation).
- [x] With VoiceOver on, an auto-advance that would fire mid-announcement is coordinated so the user is not cut off (ties into US-CC10).
- [x] Typecheck, lint, and the `RepToday` unit suite pass; accessibility behavior verified in the running app (`RepTodayUITests`, and manual VoiceOver on device).
