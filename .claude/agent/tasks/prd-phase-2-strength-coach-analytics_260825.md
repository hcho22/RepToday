...
- **The LLM transport is staged under a production hold, not live.** `proxy/` is a Cloudflare Worker with route-specific provider keys: Variety Language remains on Anthropic, while the premium Coach uses OpenAI `gpt-5.6-luna`; the Worker stores no request or response content. The guarded launch stopped before provider/client secrets or custom-domain attachment; live model QA remains pending (current evidence: `artifacts/reports/coach-production/settings-normalization.md`). The iOS Coach call path is wired but inert while its endpoint configuration is empty. `Services/Language/` separately composes an optional LLM slice over a deterministic template (`VarietyLanguageResolver`, provider `nil` in MVP).
