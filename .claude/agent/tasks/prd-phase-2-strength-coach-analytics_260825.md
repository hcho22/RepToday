...
- **The operator Coach transport is deployed; stronger client authentication is prepared locally.** `proxy/` is a Cloudflare Worker with route-specific provider keys: Variety Language remains on Anthropic, while the premium Coach uses OpenAI `gpt-5.6-luna`; the Worker stores no request or response content. The operator-only endpoint is deployed at `https://coach.reptoday.app/coach`, where missing/wrong authorization and authorized malformed-input probes passed without a model call. No real model reply or shipped-client success has been verified. The captain-selected App Attest/StoreKit implementation is prepared locally but has not been migrated; ordinary iOS endpoint and secret settings remain empty pending reviewed migration and genuine-device QA. `Services/Language/` separately composes an optional LLM slice over a deterministic template (`VarietyLanguageResolver`, provider `nil` in MVP).
...
**Hosted-policy follow-through (2026-09-01):** the canonical source for `https://reptoday.app/privacy` is tracked at `gtm/03-site/privacy.html`, alongside the complete static Pages source and its publication-gated `DEPLOY.md`. The policy mirrors the in-app disclosure's OpenAI transfer, standard abuse-monitoring retention of up to 30 days, and separate rotating Coach safety pseudonym while preserving the visible engineering-draft/legal-review warning. Source changes do not deploy production; publication still requires captain approval and authenticated access to the existing Cloudflare Pages project.
