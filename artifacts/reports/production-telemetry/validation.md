...
- `subscribe` covers both sources the funnel names: a direct paid grant from `PaywallViewModel`, and a verified first positive-price StoreKit renewal whose signed transaction history proves an introductory free-trial origin from `TrialConversionObserver`. The conversion detector deliberately does not classify a renewal when StoreKit omits a price or presents an unverified history row that leaves the boundary unprovable; a signed zero price remains known nonpaid. Production counts therefore remain a conservative lower bound rather than turning an ambiguous renewal into a conversion.

The StoreKit conversion observer is additive and writes only its bounded set of qualifying conversion transaction ids plus signed purchase timestamps as ordering metadata (newest 32; no receipt, product, price, or transaction history). It does not change the durable analytics queue/retry transport added here. The removed public `gtm/` package was not recreated or edited; the already-approved collecting App Store privacy posture remains the controlling disclosure.
