...
The StoreKit conversion observer is additive and writes only its bounded set of qualifying conversion transaction ids (newest 32; no receipt, product, price, date, or transaction history). It does not change the durable analytics queue/retry transport added here. The removed public `gtm/` package was not recreated or edited; the already-approved collecting App Store privacy posture remains the controlling disclosure.
