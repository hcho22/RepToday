...
Before use of the premium AI Coach under an unacknowledged disclosure contract, a plain, unavoidable disclosure states that a message **plus a short summary of training context** is sent to OpenAI to answer; Rep Today's proxy stores no content; OpenAI may retain prompt and reply content in abuse-monitoring logs for up to 30 days under standard retention; and a separate random Coach code is sent for abuse prevention instead of a Rep Today identity and rotates on account deletion. This is the one honest break in Rep Today's on-device privacy posture.
It is a consent gesture: the user taps **I understand** for the coach to become usable, or **Not now** to back out sending nothing.

## What shipped

- **`CoachDataDisclosureView`** (`Views/Coach/CoachDataDisclosureView.swift`): the contract-versioned pre-use disclosure, presented as an overlay layer (not a `.sheet`, so the entrance stills under Reduce Motion), `.isModal` for VoiceOver focus-trapping, 60pt controls, a `ScrollView` for Dynamic Type, `Theme` tokens only. Its copy lives in **`CoachDataDisclosureCopy`** - the single source both the modal and the Settings entry read, so they cannot drift.
...
- **`CoachView` orchestration** (`Views/Coach/CoachView.swift`): reads `@Environment(AppState.self)` (optional, like `ActiveSessionView`), and in `onAppear` either opens the gate silently (current version acknowledged) or presents the disclosure. "I understand" grants consent + persists the current version + dismisses; "Not now" `dismiss()`es the coach and records nothing, so it re-shows on the next open.
- **`AppState` versioned acknowledgement** (`Utilities/AppState.swift`): `acknowledgedCoachDataSharingDisclosureVersion` plus `hasAcknowledgedCoachDataSharing` / `shouldShowCoachDataDisclosure` / `markCoachDataSharingAcknowledged()`, UserDefaults-backed, default not-acknowledged. A matching version survives relaunch; a legacy/mismatched value or successful account deletion re-arms the disclosure. It is recorded **only on explicit acknowledgement** (never on presentation) and remains **independent of** `analyticsEnabled`.
...
- The consent gesture is **disclosure-then-acknowledge** (an explicit "I understand" for the current contract before the first request, "Not now" to back out sending nothing). The acknowledgement persists across relaunches but is account-scoped and contract-versioned, so deletion or a materially changed disclosure requires it again.
...
- The canonical public-site source is tracked under `gtm/03-site/`: `DEPLOY.md`, `_headers`, both landing variants, `privacy.html`, and four A/B screenshots. The policy's optional-AI-Coach section covers the message and derived context sent to OpenAI, Rep Today's zero-content-storage proxy, standard abuse-monitoring retention of up to 30 days, and the separate rotating safety pseudonym. The landing variants' AI, network, Premium, and data FAQ copy is synchronized so a full-directory deploy cannot restore stale public claims.
- **Publication gate:** source ownership does not authorize deployment. Publishing the canonical source requires captain approval plus authenticated access to the existing `reptoday-site` Cloudflare Pages project; `gtm/03-site/DEPLOY.md` records that gate.
