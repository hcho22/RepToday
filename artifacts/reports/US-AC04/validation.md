...
- The canonical public-site source is maintained outside this repository in the private Mandu repository at `projects/RepToday/gtm/03-site/`. Its policy covers the message and derived context sent to OpenAI, Rep Today's zero-content-storage proxy, standard abuse-monitoring retention of up to 30 days, and the separate rotating safety pseudonym. The public RepToday repository intentionally does not duplicate those site sources.
- **Publication gate:** source ownership does not authorize deployment. Publishing the canonical source requires captain approval plus authenticated access to the existing `reptoday-site` Cloudflare Pages project; the external site's deployment instructions record that gate.
