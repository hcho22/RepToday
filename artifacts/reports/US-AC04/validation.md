...
- The public policy's source is no longer external to the repository. The minimum complete Cloudflare Pages source was restored from the last tracked pre-removal tree under `gtm/03-site/`: both live root variants, `privacy.html`, `_headers`, and `DEPLOY.md` (generated screenshots and the rest of the retired marketing package remain omitted). The policy now has a dedicated optional-AI-Coach section covering the message and derived context sent to OpenAI, Rep Today's zero-content-storage proxy, standard abuse-monitoring retention of up to 30 days, and the separate rotating safety pseudonym. The landing variants' AI, network, Premium, and data FAQ copy was synchronized so a full-directory deploy cannot restore stale public claims.
- **Publication gate:** this repository edit did not deploy the site. `https://reptoday.app/privacy` was read back before the edit and still served the pre-Coach draft (apart from Cloudflare's email-obfuscation injection). Publishing the corrected source requires captain approval plus authenticated access to the existing `reptoday-site` Cloudflare Pages project; `gtm/03-site/DEPLOY.md` records that gate.
