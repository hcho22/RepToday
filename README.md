...
| `.claude/agent/tasks/prd-round-cap-wide-circuits.md` | A fourth, in-progress PRD - caps every training-block exercise at **2-4 rounds** (`US-RC##`) and fills a longer session by adding distinct movements (a strength pattern's further progression chains, as accessories) instead of more rounds. US-RC01 (the engine core) has landed; US-RC02-US-RC05 (a standing regression guard, accessory progression parity, pinning the accepted depth mismatch, and ADR-0004 + the `CONTEXT.md` term) remain open. |
