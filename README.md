...
Only needed when working on `convex/` - the iOS app builds, runs, and tests without any of it. The app's transport to this sink exists (US-T04's `LiveAnalyticsService`), and the first emission sites now call it in a Debug build - app entry (US-T07), the onboarding funnel (US-T08), the Ready Screen (US-T09), the session lifecycle (US-T10), and the weekly rollup (US-T11's `week_active`) - while the rest (US-T12, the monetization funnel) is still ahead and a Release build stays inert; the other caller is US-T06's Debug-only, launch-argument-gated XCUITest probe.
