...
7. **Assemble + fit timing** (even-round circuit model, US-CC03/US-CC04) - always open with a warm-up, add a cooldown over 10 min, and land within ±1 min of the requested time. A training block is a circuit of **even rounds**: every exercise in it carries the same set count (the block's round count), so "Round N of M" is well-defined. A set is priced as a fixed setup cost plus the per-unit work of the target actually prescribed (a per-side hold costing both sides), so a grown or seeded target is planned as the longer session it really is. Two rest gaps carry the timing fit: a fixed ~15s between-station transition inside a round, and a bounded between-round rest (30-75s) that the fit tunes as its **primary lever**, falling back to whole rounds or whole exercises - it never touches the Step 6 per-set target and never makes a block uneven. The warm-up and cooldown bookends are one set per exercise - a stretch is never multi-set - so a longer session grows them only by adding distinct movements.
