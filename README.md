...
- **Emitting where the funnel begins** - the pipeline is complete, consented, and gated, and the first call sites now feed it: app entry (US-T07), the onboarding funnel (US-T08), and the Ready Screen (US-T09). The rest (US-T10 through US-T12) are still ahead, and every emission is read against the opt-out flag afresh, so an opted-out build sends nothing either way.
...
Only needed when working on `convex/` - the iOS app builds, runs, and tests without any of it. The app's transport to this sink exists (US-T04's `LiveAnalyticsService`), and the first emission sites now call it in a Debug build - app entry (US-T07), the onboarding funnel (US-T08), and the Ready Screen (US-T09) - while the rest (US-T10 through US-T12) are still ahead and a Release build stays inert; the other caller is US-T06's Debug-only, launch-argument-gated XCUITest probe.
