...
> The previous app (XP/badges/streaks, SwiftData, and its legacy AI services) was removed and lives only in git history as reference. The separately designed Phase-2 premium Coach described below is implemented but remains inert until its proxy endpoint is deployed and configured.
...
- **The hosted policy source lives here** - `gtm/03-site/privacy.html` carries the same Coach boundary and deploys with the static `reptoday.app` site. It remains visibly marked as an engineering draft that needs qualified legal review; editing the source does not authorize or perform a production deployment.
...
├── gtm/03-site/             # Minimum deployable reptoday.app static source: root variants, hosted privacy policy, shared headers, and the publication-gated deploy guide
...
| `gtm/03-site/DEPLOY.md` | The existing Cloudflare Pages workflow for the public static site, including the source inventory and the production publication/credential gate. |
