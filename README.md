...
| `.claude/agent/tasks/prd-round-cap-wide-circuits.md` | A fourth, in-progress PRD - caps every training-block exercise at **2-4 rounds** (`US-RC##`) and fills a longer session by adding distinct movements (a strength pattern's further progression chains, as accessories) instead of more rounds. US-RC01 (the engine core) and US-RC02 (the standing "2-4 rounds, always" regression guard) have landed; US-RC03-US-RC05 (accessory progression parity, pinning the accepted depth mismatch, and ADR-0004 + the `CONTEXT.md` term) remain open. |
