...
- **Best-effort and bounded** - Coach turns use the content-stateless Cloudflare proxy's `POST /coach` route and a source-pinned OpenAI `gpt-5.6-luna` Responses API call. The request has a 30-second timeout and 1024-token output ceiling, and any failure leaves the free core loop untouched. The operator-only endpoint is deployed, but ordinary iOS builds keep their Coach endpoint and secret empty while the selected App Attest/StoreKit migration and genuine-device/model QA remain pending, so those builds show the Coach as unavailable.
...
- **Coach data is disclosed before use** - a Coach turn sends the user's message, the audited non-identifying training summary, and a separate random Coach abuse-prevention pseudonym to Rep Today's proxy, which forwards the pseudonym as OpenAI's `safety_identifier`. The proxy stores no request or response content and sets `store: false`; under standard retention, OpenAI may still retain the prompt and reply in abuse-monitoring logs for up to 30 days. When Apple verification is enabled, Rep Today retains only bounded public verification keys, counters, expiry fields, and replay-prevention metadata, with a 30-day inactivity expiry. The pseudonym is not the telemetry `installId` or an account value, remains stable across launches, and rotates on account deletion. The acknowledgement is account-scoped and versioned: deletion clears it, and a materially changed disclosure requires acknowledgement again.
...
| **LLM proxy** | `proxy/` is a content-stateless Cloudflare Worker: Anthropic `claude-opus-4-8` remains independently configurable for deferred Variety Language, while the premium Coach is source-pinned to OpenAI `gpt-5.6-luna`; the prepared App Attest gateway retains only bounded security metadata, and neither route is behind the deterministic core loop |
| **Bundle ID** | `com.reptoday.app` |

The Phase-2 premium Coach is implemented, and its operator-only endpoint is deployed behind a server-held bearer. Ordinary iOS builds remain unconfigured while the selected App Attest/StoreKit gateway awaits reviewed migration and genuine-device/model QA. The Coach does language plus bounded preference offers only; the deterministic on-device engine remains sovereign over every workout and safety filter.
...
├── tools/                   # Release/telemetry tooling, Coach operator and held-migration helpers, and the offline US-T13 reconciliation harness
├── proxy/                   # Content-stateless key-holding Worker: deployed operator Coach entry plus the prepared App Attest/StoreKit gateway
...
| `.claude/agent/tasks/prd-phase-2-strength-coach-analytics_260825.md` | A fifth, complete PRD - makes the earned Strength Phase real and visible, adds the premium Coach and its two-writer safety boundary (ADR-0005), and adds premium strength-journey analytics. The operator-only Coach endpoint is deployed with no live model reply verified; ordinary iOS builds remain inert pending the selected stronger-authentication migration and genuine-device QA. |
