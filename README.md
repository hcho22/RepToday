...
So every launch in that suite carries consent-off **or** the probe, and neither is not representable: the postures are a `TelemetryPosture` enum behind one sanctioned launch helper, with a runtime check for a launch that bypasses it - run on entry to every launch as well as at teardown, and resting on `setUp` terminating a leftover app.
That check **detects** a bypass rather than preventing one, and does not detect all of them: a bare `app.launch()` before a helper launch is caught, one standing alone as a test's only launch is not, and the guard's own cell enumeration is not the complete account of bypasses it once claimed to be.
The wrapper that makes a raw `XCUIApplication` unreachable is the answer, and it is a precondition on US-T07 rather than a contingency.
