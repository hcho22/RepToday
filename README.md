...
- **Coach data is disclosed before use** - a Coach turn sends the user's message, the audited non-identifying training summary, and a separate random Coach abuse-prevention pseudonym to Rep Today's proxy, which forwards the pseudonym as OpenAI's `safety_identifier`. The proxy stores no request or response content and sets `store: false`; under standard retention, OpenAI may still retain the prompt and reply in abuse-monitoring logs for up to 30 days. The pseudonym is not the telemetry `installId` or an account value, remains stable across launches, and rotates on account deletion. The acknowledgement is account-scoped and versioned: deletion clears it, and a materially changed disclosure requires acknowledgement again.
