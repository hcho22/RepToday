...
- **Emitting where the funnel begins** - the pipeline is complete, consented, and gated, and the first call sites now feed it: app entry (US-T07) and the onboarding funnel (US-T08). The rest (US-T09 through US-T12) are still ahead, and every emission is read against the opt-out flag afresh, so an opted-out build sends nothing either way.
