...
- **Best-effort and bounded** - Coach turns use the content-stateless Cloudflare proxy's `POST /coach` route and a source-pinned OpenAI `gpt-5.6-luna` Responses API call. The request has a 30-second timeout and 1024-token output ceiling, and any failure leaves the free core loop untouched. The deployed gateway accepts Production StoreKit proof; current undeployed source also verifies matched Apple Sandbox proof for TestFlight. Ordinary iOS builds keep their Coach endpoint and secret empty pending a separately authorized rollout and genuine-device/TestFlight/model QA, so those builds show the Coach as unavailable. See `docs/coach-runtime-authentication.md` for the authoritative runtime status.
...
- **Account deletion is separate from subscription cancellation** - Profile -> Settings -> Delete Account permanently removes the account data named in its final confirmation, but does not cancel an App Store subscription. Both the Apple-linked and local-only confirmation variants warn about that before the destructive action; the subscription must be managed or canceled separately through Apple. Deletion also unlinks the local Coach verification key and, when the verification transport is available, makes a bounded signed attempt to remove its server record; an offline record expires after 30 days without accepted verification activity.
...
- **Coach data is disclosed before use** - a Coach turn sends the user's message, the audited non-identifying training summary, and a separate random Coach abuse-prevention pseudonym to Rep Today's proxy, which forwards the pseudonym as OpenAI's `safety_identifier`. The proxy stores no request or response content and sets `store: false`; under standard retention, OpenAI may still retain the prompt and reply in abuse-monitoring logs for up to 30 days. The deployed Apple-verification gateway retains only bounded public verification keys, counters, expiry fields, and replay-prevention metadata, with a 30-day inactivity expiry. The pseudonym is not the telemetry `installId` or an account value, remains stable across launches, and rotates on account deletion. The acknowledgement is account-scoped and versioned: deletion clears it, and a materially changed disclosure requires acknowledgement again.
...
| **LLM proxy** | `proxy/` is a content-stateless Cloudflare Worker: Anthropic `claude-opus-4-8` remains independently configurable for deferred Variety Language, while the premium Coach is source-pinned to OpenAI `gpt-5.6-luna`; the deployed App Attest gateway retains only bounded security metadata, and neither route is behind the deterministic core loop |
| **Bundle ID** | `com.reptoday.app` |

The Phase-2 premium Coach is implemented; its authoritative deployment and QA status lives in `docs/coach-runtime-authentication.md`. The Coach does language plus bounded preference offers only; the deterministic on-device engine remains sovereign over every workout and safety filter.
...
├── tools/                   # Release/telemetry tooling, guarded Coach runtime helpers, and the offline US-T13 reconciliation harness
├── proxy/                   # Content-stateless key-holding Worker with the deployed App Attest/StoreKit Coach gateway
...
├── docs/                    # Implementation log, test-coverage map, Coach runtime-auth reference, asset-attribution ledger
...
└── CLAUDE.md                # @AGENTS.md pointer; edit AGENTS.md
```
...
| `.claude/agent/tasks/prd-funnel-instrumentation_260803.md` | A second, in-progress PRD - anonymous product telemetry for the 90-day PMF test, as `US-T##` stories. The analytics seam (US-T02), Convex sink (US-T03), anonymous install identity (US-T05), URLSession transport (US-T04), opt-out/disclosure (US-T06), all 13 emission sites (US-T07...T12), and the later trial-conversion emission have landed. Delivery now uses a bounded durable outbox, an iOS background-execution lease, foreground/relaunch recovery, and sink-side `eventId` idempotency. US-T14 abuse-hardened `POST /logEvent`; Release targets production only with private Keychain-backed token injection. US-T13 (ground-truth reconciliation) remains the one open story. |
...
| `.claude/agent/tasks/prd-phase-2-strength-coach-analytics_260825.md` | A fifth, complete PRD - makes the earned Strength Phase real and visible, adds the premium Coach and its two-writer safety boundary (ADR-0005), and adds premium strength-journey analytics. Current Coach runtime and QA status is owned by `docs/coach-runtime-authentication.md`. |
| `CLAUDE.md` | Repo conventions and architecture for contributors and AI assistants - an `@AGENTS.md` pointer; edit `AGENTS.md`, which links detailed reference docs. |
...
US-T04 added the behavioural boundary suite (`convex/http.test.ts`), and the US-T13 harness now also tests its indexed internal read (`convex/reconcile.query.test.ts`) and pure tabulator (`convex/reconcile/tabulate.test.ts`). All run in process against no deployment; `.github/workflows/ci.yml` runs them (with `npm run typecheck`) on every PR into `main`. Production operators use `tools/validate-production-telemetry.sh` for the authenticated sink boundary and stable-`eventId` duplicate replay, and `tools/validate-release-telemetry-client.sh` for the real Release client. The private archive command invokes the first check as a mandatory backend-first prerequisite; deployment steps and sanitized results are in `artifacts/reports/production-telemetry/validation.md`.

### Continuous integration

`.github/workflows/ci.yml` gates every PR into `main` (and pushes to it) on both toolchains in parallel jobs: `sink` (ubuntu: `npm ci` + `npm run typecheck` + `npm run test`) and `ios` (macos-15, Xcode 16.4, iPhone 16 / iOS 18.5 simulator - `xcodegen generate`, fixture-tests the shared app-bundle metadata validator, then builds and **runs** the `RepToday` unit suite, validates the processed `RepToday.app/Info.plist`, and performs a **build-for-testing-only** compile of the `RepTodayUITests` scheme, since XCUITests launch the app out of process and are slow/flaky on a hosted runner). The same validator runs against the archived app in `tools/archive-release.sh`; `tools/validate-app-bundle-metadata.py` owns that metadata contract. CI needs no secrets: the app's CloudKit/HealthKit/Sign in with Apple entitlements make a fully unsigned build crash the app-hosted test runner, so it uses cert-less ad-hoc signing (`CODE_SIGN_IDENTITY=-`, team/profile cleared) which loads entitlements on the simulator without any certificate. The local `no-mistakes` pipeline is unchanged.
