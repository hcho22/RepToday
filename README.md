...
The Phase-2 premium Coach and its App Attest/production-StoreKit runtime gateway are implemented and deployed. Ordinary iOS builds remain unconfigured pending verified TestFlight proof-format compatibility and genuine-device/model QA; deployment does not establish any of those outcomes. The Coach does language plus bounded preference offers only; the deterministic on-device engine remains sovereign over every workout and safety filter.
...
| `.claude/agent/tasks/prd-phase-2-strength-coach-analytics_260825.md` | A fifth, complete PRD - makes the earned Strength Phase real and visible, adds the premium Coach and its two-writer safety boundary (ADR-0005), and adds premium strength-journey analytics. The App Attest/production-StoreKit gateway is deployed with no genuine Apple, purchase, model, or shipped-client success verified; ordinary iOS builds remain inert pending verified TestFlight proof-format compatibility. |
