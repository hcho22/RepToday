...
Across single-focus, blend, cold-start, and Return sessions, strength leads; mobility is supporting only - the structural warm-up, the cooldown, and a minority accessory block (Movement Practice).
...
- A blend is a fixed strength-dominant envelope with mobility a minority accessory, sized by a base share rather than by staleness (US-002).
...
- **Desk workers (`sitsLong`) lose a dedicated same-day mobility-relief day.** `sitsLong` now only enlarges the mobility accessory's volume within a strength-led blend; it never makes mobility the lead. The same-day-relief differentiator the co-primary framing leaned on is given up on the engine side.
