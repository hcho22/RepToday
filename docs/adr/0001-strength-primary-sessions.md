...
- **Desk workers (`sitsLong`) lose a dedicated same-day mobility-relief day.** `sitsLong` originally only enlarged the mobility accessory's volume within a strength-led blend; it never made mobility the lead. *(US-M01: the accessory block is gone, so `sitsLong` no longer sizes anything; US-M03 repurposed it to bias bookend selection toward posture/hip openers, still never sizing a block.)* The same-day-relief differentiator the co-primary framing leaned on is given up on the engine side.
