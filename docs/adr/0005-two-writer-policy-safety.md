...
2. **Overlay onto the current policy, re-read and written atomically.** `CoachSessionPolicyService.applyProposal` applies the proposal *onto* the freshest persisted policy (`SessionPolicy.applyingCoachProposal`), rather than building a fresh policy or replacing unmentioned levers. A de-load that landed since the user last spoke to the coach is already in that policy and is carried through untouched. The read-overlay-write runs through the store's `update(for:transform:)` seam, which performs all three steps under the store's own isolation (the in-memory store is an `actor`; `CoreDataSessionPolicyStore` runs them inside a single `context.perform` block), so a deterministic safety write cannot interleave between the coach's read and its save - the overlay is always built on the truly-latest policy, not a stale snapshot.
...
- **Last-writer-wins with a stored baseline / timestamp comparison.** Rejected: it requires persisting extra state to detect "a safety move is in force" and is fragile across reads. The overlay + only-downward rule needs no baseline; the read-then-save window a naive re-read would leave open is closed instead by the store's atomic `update(for:transform:)` seam (part 2), so the overlay is always built on the truly-latest policy.
