...
/// The one place that decides where a test suite's reviewer evidence is written.
///
/// Every evidence suite produces and asserts its artifacts on every run, but where the bytes land is
/// opt-in:
///
/// - by default, a per-run temporary directory, so a plain `test` never leaves the worktree dirty
///   with regenerated artifacts and never depends on the machine the evidence was first captured on;
/// - `REPTODAY_WRITE_EVIDENCE=1` points the same writes at the repo's own `artifacts/reports/`,
///   which is how the committed images and transcripts the PRD and `docs/test-coverage.md` cite are
///   produced - deliberately, by these tests, rather than copied there by hand;
...
enum EvidenceOutput {
    ...
    /// Each story keeps its evidence in its own folder, so an artifact belonging to one story's work
    ...
    /// Writes a text artifact into `story`'s evidence directory, resolving the destination and creating
    /// it through the same path the image write uses, so a suite never re-implements the
