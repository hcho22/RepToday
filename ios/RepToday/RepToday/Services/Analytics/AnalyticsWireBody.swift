...
/// differ too - the sink's contract is `{eventId, name, installId, clientTs, props}`, where
/// `eventId` is the stable idempotency key reused by retries, `name` is the event name's snake_case
/// raw value, and `clientTs` is the event's `timestampMs`.
///
...
struct AnalyticsWireBody: Encodable, Equatable {
    /// Stable across retries of one accepted event; the sink deduplicates on it.
    let eventId: String
    ...
    init(event: AnalyticsEvent, installId: String, eventId: String) {
    ...
    static func encode(_ event: AnalyticsEvent, installId: String, eventId: String) throws -> Data {
        let encoder = JSONEncoder()
        encoder.outputFormatting = [.sortedKeys, .withoutEscapingSlashes]
        return try encoder.encode(AnalyticsWireBody(event: event, installId: installId, eventId: eventId))
    }
}
