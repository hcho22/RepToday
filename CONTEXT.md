...
**Partially landed:** US-CC01 shipped the first player slice - a rep-based **strength/primal** training set now counts down on an auto-advancing **work window** (`ActiveSessionViewModel.workWindow`, `ActiveSessionView.WorkWindowCountdownView`) that records the set at zero and flows into the *existing* US-K02 rest with no tap, plus a prominent **Done** to advance early; the window reads the engine's already-planned per-set seconds (`SessionAssembly.workSecondsPerSet`). **US-CC03/US-CC04 have landed** (engine-only, ADR-0003): `SessionAssembly` now sizes a training block as a circuit of a **uniform round count** (every station's `sets` equal) and lands the ±1 min target by tuning a **bounded between-round rest** (30-75s) as the primary fit lever, with a fixed ~15s between-station transition as the distinct second gap - the per-exercise set-adjust lever is retired (`SessionAssembly.blockSeconds`/`setRoundsAndRest`). **US-CC02 has landed** (player-only): the strength / dedicated-primal training block is played as a **circuit of rounds** - the player rotates one set of each station per round (A, B, C, A, B, C, ...) via `ActiveSessionViewModel.nextPosition(fromIndex:set:)` and surfaces **"Round N of M"** (`currentRound`/`circuitRoundCount`), warm-up/cooldown bookends stay linear (no round), and aggregate logging is unchanged. The rest of the model - swap-across-rounds (US-CC07), hands-free bookends (US-CC05), the US-CC08 generous-pace recalibration of `workSecondsPerSet`, and the escape hatches beyond Done - is **not yet built**, so warm-up/cooldown holds and non-training sets still use the manual player. Do not read the fuller description above as current beyond the strength work window, its circuit rotation, and the engine's even-round sizing.
